...
def upload_to_youtube(file_path, title):
    ...
    channel = youtube.channels().list(part="id,snippet", mine=True).execute().get("items", [])
    if not channel:
        raise RuntimeError("OAuth succeeded but no YouTube channel was found.")

    print(f"Authenticated channel: {channel[0]['snippet']['title']}")
