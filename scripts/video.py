...
MH_PROJECT_URL = "https://api.magichour.ai/v1/video-projects"  # GET status here, not on the create URL
...
def _submit_job(image_url, prompt, duration, api_key):
    """Submit video job using image_file_path (CORRECT field name per Magic Hour API)."""
    headers = {
    ...
    payload = {
        "model": MH_MODEL,
        "assets": {"image_file_path": image_url},  # <-- Correct field!
...
def _poll_job(job_id, api_key):
    ...
    while time.time() - start < POLL_TIMEOUT:
        ...
        try:
            r = requests.get(f"{MH_PROJECT_URL}/{job_id}", headers=headers, timeout=30)
            if r.status_code != 200:
                continue
            data = r.json()
