...
LIPSYNC_SPACES = [
    s.strip() for s in os.getenv(
        "LIPSYNC_SPACES",
        "manavisrani07/gradio-lipsync-wav2lip,lmh07072000/gradio-lipsync-wav2lip"
    ).split(",") if s.strip()
]
...
def clean_script_for_speech(text):
    ...
    # Edge-TTS can make a noticeably long pause at sentence-ending punctuation.
    # For this short-form voiceover we want a continuous, natural flow, so turn
    # periods/question marks/exclamation marks into short comma-level pauses.
    text = re.sub(r"[.!?]+", ",", text)
    text = re.sub(r"[;:]+", ",", text)
...
def try_lipsync(character_image, audio_path):
    ...
    for space in LIPSYNC_SPACES:
        ...
        try:
            download_dir = OUTPUT_DIR / "gradio_downloads"
            ...
            # The current Wav2Lip Spaces expose /generate with the inputs:
            # video/image, audio, checkpoint, no_smooth, resize_factor, and padding.
            # Use positional arguments here because some Spaces don't accept the
            # old face= keyword even though the UI still shows a face/video input.
            job = client.submit(
                handle_file(str(character_image)),
            ...
            if not local_or_url:
                print("Lipsync Space returned no file. Trying next Space.")
                continue

            local_path = Path(str(local_or_url))
            if not local_path.exists() or local_path.stat().st_size < 10000:
                print(f"Lipsync result is not a usable local file: {local_or_url}")
                print("Trying next Space.")
                continue

            normalized = OUTPUT_DIR / f"lipsync_normalized_{int(time.time())}.mp4"
            normalize_lipsync_video(local_path, normalized)
            print(f"Lipsync video ready: {normalized}")
            return normalized
