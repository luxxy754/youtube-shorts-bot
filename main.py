    ...
    for token_idx, token in enumerate(tokens_to_try):
        for space_id in HF_SPACE.split(","):
            ...
            try:
                ...
                result = None
                # Try common api_name first (works for most Gradio Spaces with named endpoints)
                for kwargs in (
                    {"api_name": "/predict"},
                    {"fn_index": 0},  # fallback: positional call, no keyword args allowed here
                ):
                    try:
                        if "fn_index" in kwargs:
                            result = client.predict(prompt_text, fn_index=kwargs["fn_index"])
                        else:
                            result = client.predict(prompt_text, api_name=kwargs["api_name"])
                        if result:
                            break
                    except Exception as inner_e:
                        print(f"  -> attempt with {kwargs} failed: {inner_e}")
                        continue
